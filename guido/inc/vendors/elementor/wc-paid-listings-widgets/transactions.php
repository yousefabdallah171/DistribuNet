...
class Guido_Elementor_Transactions extends Elementor\Widget_Base {
	...
	public function get_title() {
        return esc_html__( 'Apus Transactions', 'guido' );
    }
	...
	protected function render() {
        ...
        extract( $settings );
        ?>
        <h1 class="title-profile"><?php esc_html_e( 'Transactions', 'guido' ) ; ?></h1>
        <div class="box-white-dashboard">
            <div class="inner-list">
                ...
                <?php if ( ! is_user_logged_in() ) {
                        ...
                        <div class="widget-user-packages <?php echo esc_attr($el_class); ?>">
                            <div class="widget-content table-responsive">
                                <table class="user-transactions">
                                    <thead>
                                        <tr>
                                            <td><?php esc_html_e('Order ID', 'guido'); ?></td>
                                            <td><?php esc_html_e('Package', 'guido'); ?></td>
                                            <td><?php esc_html_e('Amount', 'guido'); ?></td>
                                            <td><?php esc_html_e('Date', 'guido'); ?></td>
                                            <td><?php esc_html_e('Payment Mode', 'guido'); ?></td>
                                    ...
                                    <tbody>
                                        <?php while ($trans_loop->have_posts()) : $trans_loop->the_post();
                                            ...
                                            if ($trans_status == 'completed') {
                                                $status_txt = esc_html__('Successfull', 'guido');
                                                $status_class = 'success';
                                            } else if ($trans_status == 'processing') {
                                                $status_txt = esc_html__('Processing', 'guido');
                                                $status_class = 'pending';
                                            } else if ($trans_status == 'refunded') {
                                                $status_txt = esc_html__('Refunded', 'guido');
                                                $status_class = 'pending';
                                            } else {
                                                $status_txt = esc_html__('Pending', 'guido');
                                                $status_class = 'pending';
                                            }
                                            ...
                                            if ($payment_mode == 'cod') {
                                                $payment_mode = esc_html__('Cash on Delivery', 'guido');
                                            }
                                        ?>
                                            <tr>
                    ...
                    <?php } else { ?>
                        <div class="not-found"><?php esc_html_e('Don\'t have any items', 'guido'); ?></div>
                    <?php } ?>
                <?php } ?>
            </div>
        </div>
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Guido_Elementor_Transactions );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Guido_Elementor_Transactions );
}