...
class Guido_Elementor_Tabs extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Apus Tabs', 'guido' );
    }
    ...
    protected function register_controls() {
        ...
        $tabs->add_control(
            'content_type',
            [
                'label'       => esc_html__( 'Content Type', 'guido' ),
                'type'        => Controls_Manager::SELECT,
                'default'     => 'template',
                'options'     => [
                    'template' => esc_html__( 'Template', 'guido' ),
                    'editor'   => esc_html__( 'Editor', 'guido' ),
                ],
                'label_block' => 'true',
            ]
        );
        ...
        $tabs->add_control(
            'item_template_id',
            [
                'label'       => esc_html__( 'Choose Template', 'guido' ),
                'type'        => Controls_Manager::SELECT,
                'default'     => '0',
                'options'     => $options,
                'types'       => $types,
        ...
        $tabs->add_control(
            ...
            [
                ...
                'default'    => esc_html__( 'Tab Item Content', 'guido' ),
                'dynamic' => [
                    'active' => true,
                ],
                'condition'   => [
        ...
        $this->add_control(
            'title', [
                'label' => esc_html__( 'Title', 'guido' ),
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__( 'Enter title here' , 'guido' ),
                'label_block' => true,
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => array(
                    ...
                    'style5' => esc_html__('Style 5', 'guido'),
                ),
                'default' => ''
            ]
        );
                                ...
                                switch ( $tab[ 'content_type' ] ) {
                                    case 'template':
                                        ...
                                        if ( '0' !== $tab['item_template_id'] ) {
                                            $item_template_id = guido_get_lang_post_id($tab['item_template_id'], 'elementor_library');
                                            ...
                                            if ( ! empty( $template_content ) ) {
                                                ...
                                                if ( Plugin::$instance->editor->is_edit_mode() ) {
                                                    ...
                                                    $content_html .= sprintf( '<div class="guido__edit-cover" data-template-edit-link="%s"><i class="fa fa-pencil"></i><span>%s</span></div>', $link, esc_html__( 'Edit Template', 'guido' ) );
                                                }
                                            } else {
    ...
    public function no_templates_message() {
        return '<div class="no-template-message"><span>' . esc_html__( 'Template is not defined.', 'guido' ) . '</span></div>';
    }

    public function no_template_content_message() {
        return '<div class="no-template-message"><span>' . esc_html__( 'The tabs are working. Please, note, that you have to add a template to the library in order to be able to display it inside the tabs.', 'guido' ) . '</span></div>';
    }
    ...
    public function empty_templates_message() {
        $output = '<div id="elementor-widget-template-empty-templates">';
            ...
            $output .= '<div class="elementor-widget-template-empty-templates-title">' . esc_html__( 'You Haven’t Saved Templates Yet.', 'guido' ) . '</div>';
            $output .= '<div class="elementor-widget-template-empty-templates-footer">';
                $output .= esc_html__( 'What is Library?', 'guido' );
                $output .= '<a class="elementor-widget-template-empty-templates-footer-url" href="https://go.elementor.com/docs-library/" target="_blank">' . esc_html__( 'Read our tutorial on using Library templates.', 'guido' ) . '</a>';
            $output .= '</div>';
        $output .= '</div>';
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Plugin::instance()->widgets_manager->register_widget_type( new Guido_Elementor_Tabs );
} else {
    Plugin::instance()->widgets_manager->register( new Guido_Elementor_Tabs );
}