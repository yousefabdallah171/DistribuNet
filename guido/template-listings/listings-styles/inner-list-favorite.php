...
<article <?php post_class('map-item listing-grid listing-item listing-item-favorite listing-favorite-wrapper'); ?> <?php guido_listing_item_map_meta($post); ?>>

    <div class="listing-thumbnail-wrapper">
        <?php guido_listing_display_image( $post, 'guido-listing-grid' ); ?>
        <a href="javascript:void(0)" class="btn-remove-listing-favorite btn-remove" data-listing_id="<?php echo esc_attr($post->ID); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-listings-directory-remove-listing-favorite-nonce' )); ?>"><i class="ti-close"></i></a>
