...
var wp_listings_directory_all_loaded_icons = {};
jQuery(document).ready(function($){
    
    // custom fields
    $(document).on('click', '.wp-listings-directory-custom-field-add-available-field', function () {
        "use strict";
        ...
        var ajax_url = wp_listings_directory_customfield_common_vars.ajax_url,
                $this = $(this),
        ...
        var action = 'wp_listings_directory_custom_field_available_html';
    ...
    $(document).on('click', '.btn-add-field', function () {
        ...
        var ajax_url = wp_listings_directory_customfield_common_vars.ajax_url,
                $this = $(this),
                randid = $this.data('randid'),
                select_element = $('.wp-listings-directory-field-types'),
        ...
        action = 'wp_listings_directory_custom_field_html';
        var dataString = 'action=' + action + '&fieldtype=' + fieldtype + '&global_custom_field_counter=' + global_custom_field_counter;
    ...
    $(document).on('click', '.custom-fields-remove', function () {
        ...
        $('.custom-field-class-' + randid).slideUp("normal").promise().done(function () {
            $(this).remove();
            $('.wp-listings-directory-form-field-list ul li.'+ fieldtype).removeClass('disabled');
    ...
    $(document).on('keyup change', '.wp-listings-directory-custom-field-label', function(e){
        var parent = $(this).parents('.wp-listings-directory-custom-field-container');
        parent.find('.field-intro a b').text( '(' + $(this).val() + ')' );
    });

    $(document).on('keyup change', '.wp-listings-directory-custom-field-key', function(e){
        if (!wp_listings_directory_isArrowKey(e)) {
            var new_val = wp_listings_directory_str2url($(this).val().replace(/^[0-9]+\./, ''), 'UTF-8');
            $(this).val(new_val);
            var check = wp_listings_directory_check_key_available(new_val);
            if ( check ) {
    ...
    $(document).on('keyup change', 'input[data-general-review-key]', function(e){
        if (!wp_listings_directory_isArrowKey(e)) {
            ...
            var new_val = wp_listings_directory_str2url($(this).val().replace(/^[0-9]+\./, ''), 'UTF-8');
            $(this).val(new_val);
            var check = wp_listings_directory_check_review_key_available(new_val, $('input[data-general-review-key='+cat+']'));
            if ( check ) {
                $(this).parent().find('.error').remove();
                $(this).parent().append('<span class="error">This key is not right</span>');
            } else {
    ...
    function wp_listings_directory_isArrowKey(k_ev)
    {
    ...
    function wp_listings_directory_str2url(str, encoding, ucfirst)
    {
        str = str.toUpperCase();
        str = str.toLowerCase();
    ...
    function wp_listings_directory_check_key_available($key) {
        var $i = 0;
        $('.wp-listings-directory-custom-field-key').each(function(){
            if ( $key === $(this).val() ) {
                $i = $i + 1;
            }
        });
    ...
    function wp_listings_directory_check_review_key_available($key, $input_e) {
        var $i = 0;
    ...
    // icons
    $(function(){
        $("body").trigger("wp_listings_directory_all_loading_icons");
        ...
        var icon_type = $('input[name=_icon_type]:checked').val();
        ...
        $('input[name=_icon_type]').change(function(){
            var icon_type = $('input[name=_icon_type]:checked').val();
            $('.icon-type-wrapper').hide();
            if ( icon_type == 'font' ) {
                $('.icon-type-font').show();
        ...
        // function wpld_add_file(event, selector) {
        ...
        //     wpld_add_file(event, $(this).parent().parent());
        // });
    });
