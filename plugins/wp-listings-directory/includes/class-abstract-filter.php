...
class WP_Listings_Directory_Abstract_Filter {
		...
		if ( ! empty( $params['filter-orderby'] ) ) {
			switch ( $params['filter-orderby'] ) {
				...
				case 'published':
					...
					break;
				case 'price-lowest':
					$query_vars['meta_key'] = WP_LISTINGS_DIRECTORY_LISTING_PREFIX . 'price';
	...
	public static function get_posts_by_distance($latitude, $longitude, $distance) {
		...
		if ( apply_filters( 'wp_listings_directory_get_listings_cache_results', false ) ) {
			$to_hash         = json_encode( array($earth_distance, $latitude, $longitude, $latitude, $distance, $post_stype) );
			$query_args_hash = 'wp_listings_directory_' . md5( $to_hash . WP_LISTINGS_DIRECTORY_PLUGIN_VERSION );
	...
	public static function filter_count($name, $term_id, $field) {
		$args = array(
			'post_type' => !empty($field['for_post_type']) ? $field['for_post_type'] : 'listing',
			'post_per_page' => 1,
			'fields' => 'ids'
		);
		$params = array();
		...
		$cached_counts = (array) get_transient( 'wp_listings_directory_filter_counts' );
		if ( ! isset( $cached_counts[ $query_hash ] ) ) {
			$loop = WP_Listings_Directory_Query::get_posts($args, $params);
			$cached_counts[ $query_hash ] = $loop->found_posts;
			set_transient( 'wp_listings_directory_filter_counts', $cached_counts, DAY_IN_SECONDS );
		}
	...
	public static function filter_get_name($key, $field) {
		...
		$name = $prefix.'-'.$key;
		return apply_filters('wp-listings-directory-filter-get-name', $name, $key, $field);
	}
	...
	public static function filter_field_input($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/text' );
	}
	...
	public static function filter_date_field_input($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/text_date' );
	}
	...
	public static function filter_field_input_location($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/text_location' );
	}
    
    public static function filter_field_input_distance($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : apply_filters( 'wp_listings_directory_filter_distance_default', 50 );

		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/distance' );
	}

	public static function filter_field_year_built_range_slider($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		$min_max = WP_Listings_Directory_Query::get_min_max_meta_value(WP_LISTINGS_DIRECTORY_LISTING_PREFIX.'year_built', 'listing');
		if ( empty($min_max) ) {
			return;
		}
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/range_slider' );
	}
	...
	public static function filter_field_listing_price($instance, $args, $key, $field) {
		...
		$price_min = WP_Listings_Directory_Query::get_min_max_meta_value(WP_LISTINGS_DIRECTORY_LISTING_PREFIX.'price_to', 'listing');
		$price_max = WP_Listings_Directory_Query::get_min_max_meta_value(WP_LISTINGS_DIRECTORY_LISTING_PREFIX.'price_from', 'listing');
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/price_range_slider' );
	}

	public static function filter_field_listing_price_range($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/price_range' );
	}
	...
	public static function filter_field_checkbox($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/checkbox' );
	}
	...
	public static function filter_field_select($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/select' );
	}
	...
	public static function filter_field_taxonomy_radio_list($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/radios' );
	}
	...
	public static function filter_field_taxonomy_check_list($instance, $args, $key, $field) {
		...
		include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/check_list' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_radio_list($instance, $args, $key, $field) {
	    ...
	    include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/tax_radios' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_check_list($instance, $args, $key, $field) {
	    ...
	    include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/tax_check_list' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_select($instance, $args, $key, $field) {
	    ...
	    include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/tax_select' );
	}
	...
	public static function filter_field_location_select($instance, $args, $key, $field) {
	    ...
	    if ( $location_type === 'no' ) {
	    	...
	    	include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/regions_select' );
	    }
	}

	public static function get_the_level($id, $type = 'listing_location') {
	...
	public static function hierarchical_tax_tree($catId = 0, $depth = 0, $input_name, $key, $field, $selected, $input_type = 'checkbox') {
		...
		if ( ! is_wp_error( $terms ) && ! empty ( $terms ) ) {
			$_id = WP_Listings_Directory_Mixes::random_key();
			foreach ($terms as $term) {
			  	$checked = '';
			  	if ( !empty($selected) ) {
	...
	public static function filter_field_min_max_input($instance, $args, $key, $field) {
	    ...
	    include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/min_max_text' );
	}

	public static function filter_field_range_slider($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		$min_max = WP_Listings_Directory_Query::get_min_max_meta_value($field['id'], 'listing');
		if ( empty($min_max) ) {
			return;
		}
		//$min    = floor( $min_max->min );
	...
	public static function filter_field_number_select($instance, $args, $key, $field) {
	    ...
	    include WP_Listings_Directory_Template_Loader::locate( 'widgets/filter-fields/number_select' );
	}
}
