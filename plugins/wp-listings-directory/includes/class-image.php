...
class WP_Listings_Directory_Image {
	
	public static function init() {
		add_filter( 'upload_dir', array( __CLASS__, 'upload_dir' ) );
		// Ajax endpoints.
		add_action( 'wpld_ajax_wp_listings_directory_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
		
		// compatible handlers.
		add_action( 'wp_ajax_wp_listings_directory_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
		add_action( 'wp_ajax_nopriv_wp_listings_directory_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
	}

	public static function process_upload_file() {
		
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( __( 'You must be logged in to upload files using this method.', 'wp-listings-directory' ) );
			return;
		}
		$data = array(
			'files' => array(),
	...
	public static function prepare_uploaded_files( $file_data ) {
		...
		return apply_filters( 'wp_listings_directory_prepare_uploaded_files', $files_to_upload );
	}

	public static function upload_file( $file, $args = array() ) {
		global $wp_listings_directory_upload, $wp_listings_directory_uploading_file;
		...
		$wp_listings_directory_upload         = true;
		$wp_listings_directory_uploading_file = $args['file_key'];
		$uploaded_file              = new stdClass();

		if ( '' === $args['allowed_mime_types'] ) {
			$allowed_mime_types = self::get_allowed_mime_types($wp_listings_directory_uploading_file);
		} else {
		 ...
		 */
		$file = apply_filters( 'wp_listings_directory_upload_file_pre_upload', $file, $args, $allowed_mime_types );
		...
		if ( ! in_array( $file['type'], $allowed_mime_types, true ) ) {
			if ( $args['file_label'] ) {
				...
				return new WP_Error( 'upload', sprintf( __( '"%1$s" (filetype %2$s) needs to be one of the following file types: %3$s', 'wp-listings-directory' ), $args['file_label'], $file['type'], implode( ', ', array_keys( $allowed_mime_types ) ) ) );
			} else {
				// translators: %s is the list of allowed file types.
				return new WP_Error( 'upload', sprintf( __( 'Uploaded files need to be one of the following file types: %s', 'wp-listings-directory' ), implode( ', ', array_keys( $allowed_mime_types ) ) ) );
			}
		} else {
		...
		$wp_listings_directory_upload         = false;
		$wp_listings_directory_uploading_file = '';
	...
	public static function upload_dir( $pathdata ) {
		...
		if ( ! empty( $wp_listings_directory_upload ) ) {
			$dir = untrailingslashit( apply_filters( 'wp_listings_directory_upload_dir', 'wp-listings-directory-uploads/' . sanitize_key( $wp_listings_directory_uploading_file ), sanitize_key( $wp_listings_directory_uploading_file ) ) );
	...
	public static function create_attachment( $attachment_url, $post_id = 0 ) {
		...
		$image_sizes_remove = apply_filters( 'wp-listings-directory-create-attachment-remove-image-sizes', ['shop_thumbnail', 'shop_single', 'shop_catalog', 'woocommerce_gallery_thumbnail', 'woocommerce_single', 'woocommerce_thumbnail'] );
		...
		$attachment = array(
			'post_title'   => esc_html__('Attachment Image', 'wp-listings-directory'),
			'post_content' => '',
			'post_status'  => 'inherit',
			'post_parent'  => $post_id,
			'guid'         => $attachment_url,
	...
	public static function get_allowed_mime_types( $field = '' ) {
		$allowed_mime_types = WP_Listings_Directory_Mixes::get_image_mime_types();
		
		return apply_filters( 'wp_listings_directory_mime_types', $allowed_mime_types, $field );
	}
	...
	public static function upload_attach_with_external_url($image_url, $post_ID = 0) {
		global $wp_listings_directory_upload;
		$wp_listings_directory_upload = true;
...
WP_Listings_Directory_Image::init();