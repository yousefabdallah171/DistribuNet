...
class WP_Listings_Directory_Custom_Fields_Display {
	...
	public static function hooks() {
		$hooks = WP_Listings_Directory_Fields_Manager::get_display_hooks();
	...
	public static function display_hook($post, $current_hook) {
		$custom_fields = WP_Listings_Directory_Fields_Manager::get_custom_fields_data();
	...
	public static function display_field_data($custom_field, $post, $current_hook) {
		...
		switch ( $field_type ) {
            ...
            case 'date':
                ...
                break;
            case 'checkbox':
            	$output_value = $value ? esc_html__('Yes', 'wp-listings-directory') : esc_html__('No', 'wp-listings-directory');
            	break;
            case 'multiselect':
                if ( is_array($value) ) {
                ...
                if ( is_array($value) ) {
                	foreach ($value as $file) {
                		if ( self::check_image_mime_type($file) ) {
                			...
                			$return .= '<a href="'.esc_url($file).'">'.esc_html__('Download file', 'wp-listings-directory').'</a>';
                		}
                	}
                } elseif ( !empty($value) ) {
                	if ( self::check_image_mime_type($value) ) {
            			...
            			$return .= '<a href="'.esc_url($value).'">'.esc_html__('Download file', 'wp-listings-directory').'</a>';
            		}
                }
                $output_value = $return;
            break;
        ...
        ob_start();
        if ( $current_hook === 'wp-listings-directory-single-listing-details' ) {
            ?>
            <li>
                <?php if ( $field_name ) { ?>
        ...
        $html = ob_get_clean();
        return apply_filters( 'wp_listings_directory_display_field_data', $html, $custom_field, $post, $field_name, $output_value, $current_hook );
	}
...
WP_Listings_Directory_Custom_Fields_Display::init();