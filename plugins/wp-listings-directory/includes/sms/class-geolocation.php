...
class WP_Listings_Directory_SMS_Geolocation {
	...
	public static function init() {
		$geolocation = wp_listings_directory_get_option( 'phone_approve_default_country_code', 'geolocation' );
		$show_country_code = wp_listings_directory_get_option( 'phone_approve_show_country_code', 'show_country_code' );

		if ( $show_country_code == 'on' && $geolocation == 'geolocation' ) {
			self::get_data();
		}
	}
	...
	public static function get_data( $from_cookie = true ){
		...
		if( $from_cookie && isset( $_COOKIE['wp_listings_directory_user_ip_data'] ) && !empty( $_COOKIE['wp_listings_directory_user_ip_data']) ){
			return json_decode( stripslashes( $_COOKIE['wp_listings_directory_user_ip_data'] ), true );
		}
		...
		@setcookie( 'wp_listings_directory_user_ip_data', json_encode( $mo_data ) );
	...
	*/
	public static function get_phone_code( $country_code = '' ){
		...
		$phoneCodes = (array) include WP_LISTINGS_DIRECTORY_PLUGIN_DIR.'includes/sms/countries-phone.php';
...
WP_Listings_Directory_SMS_Geolocation::init();