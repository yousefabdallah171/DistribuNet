...
class WP_Listings_Directory_Taxonomy_Car_Feature{
	...
	public static function definition() {
		//
		$singular = __( 'Feature', 'wp-listings-directory' );
		$plural   = __( 'Features', 'wp-listings-directory' );
		...
		$rewrite_slug = get_option('wp_listings_directory_listing_feature_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-feature', 'Listing feature slug - resave permalinks after changing this', 'wp-listings-directory' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_feature', 'listing', array(
			'labels'            => apply_filters( 'wp_listings_directory_taxomony_listing_feature_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_Listings_Directory_Taxonomy_Car_Feature::init();