...
class WP_Listings_Directory_CMB2_Field_File {

	public static function init() {
		add_filter( 'cmb2_render_wp_listings_directory_file', array( __CLASS__, 'render_map' ), 10, 5 );
		add_filter( 'cmb2_sanitize_wp_listings_directory_file', array( __CLASS__, 'sanitize_map' ), 10, 4 );	
		add_filter( 'cmb2_types_esc_wp_listings_directory_file', array( __CLASS__, 'esc_escaped_value' ), 10, 4 );	
	}
	...
	public static function render_map( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		if ( is_user_logged_in() && $ajax ) {
			...
			$classes[] = 'wp-listings-directory-file-upload';
		}
			...
			<?php if ( ! empty( $files ) ) : ?>
				<?php if ( is_array( $files ) ) :?>
					<?php foreach ( $files as $key => $value ) : ?>
						<?php
							echo WP_Listings_Directory_Template_Loader::get_template_part( 'misc/uploaded-file-html', array( 'input_name' => $current_name, 'value' => $key, 'file_url' => $value, 'field' => $field ) );
						?>
					<?php endforeach; ?>
				<?php elseif ( $value = $files ) : ?>
					<?php echo WP_Listings_Directory_Template_Loader::get_template_part( 'misc/uploaded-file-html', array( 'input_name' => $current_name, 'value' => $value, 'field' => $field ) ); ?>
				<?php endif; ?>
			<?php endif; ?>
		</div>
		...
		<div class="label-can-drag">
			<div class="form-group group-upload">
		        <div class="upload-file-btn">
		            <?php if ( $multiple ) { ?>
		            	<span><?php esc_html_e('Upload Files', 'wp-listings-directory'); ?></span>
	            	<?php } else { ?>
		            	<span><?php esc_html_e('Upload File', 'wp-listings-directory'); ?></span>
	            	<?php } ?>
	...
	public static function sanitize_map( $override_value, $value, $object_id, $field_args ) {
				...
				if ( is_array($values) ) {
					...
					foreach ($values as $attachment_url) {
						if ( ! empty( $attachment_url ) && is_numeric( $attachment_url ) ) {
							...
							$attach_id = WP_Listings_Directory_Image::create_attachment($attachment_url, $object_id);
							if ( $attach_id ) {
					...
					if ( is_numeric( $values ) ) {
						...
						$attach_id = WP_Listings_Directory_Image::create_attachment( $values, $object_id );
						$url = wp_get_attachment_url( $attach_id );
						$return = $url;
					}
				}
		...
		if ( isset( $field_args['object_type'] ) && $field_args['object_type'] == 'user' ) {
			update_user_meta($object_id, $field_args['_name'].'_img', $return);
		} else {
...
WP_Listings_Directory_CMB2_Field_File::init();