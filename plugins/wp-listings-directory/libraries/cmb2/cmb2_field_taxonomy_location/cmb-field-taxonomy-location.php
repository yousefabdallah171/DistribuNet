...
/**
 * Class WP_Listings_Directory_CMB2_Field_Taxonomy_Location
 */
class WP_Listings_Directory_CMB2_Field_Taxonomy_Location {
	...
	public function __construct() {
		add_filter( 'cmb2_render_wpld_taxonomy_location', array( $this, 'render_taxonomy_location' ), 10, 5 );
		add_filter( 'cmb2_sanitize_wpld_taxonomy_location', array( $this, 'sanitize' ), 10, 4 );
		add_filter( 'cmb2_types_esc_wpld_taxonomy_location', array( $this, 'escaped_value' ), 10, 3 );
		add_filter( 'cmb2_repeat_table_row_types', array( $this, 'table_row_class' ), 10, 1 );

		// Ajax endpoints.
		add_action( 'wpld_ajax_wpld_process_change_location', array( $this, 'process_change_location' ) );
		
		// compatible handlers.
		add_action( 'wp_ajax_wpld_process_change_location', array( $this, 'process_change_location' ) );
		add_action( 'wp_ajax_nopriv_wpld_process_change_location', array( $this, 'process_change_location' ) );
	}
	...
	public function render_taxonomy_location( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		for ($i=1; $i <= $nb_fields; $i++) {
			...
			$field_name = apply_filters('wp_listings_directory_cmb2_field_taxonomy_location_field_name_'.$i, 'Country');
			...
			$a = $field_type_object->parse_args( 'wpld_taxonomy_location', array(
				'style'            => 'width: 99%',
				'class'            => 'wpld_taxonomy_location wpld_taxonomy_location'.$i,
	...
	public function setup_admin_scripts() {
		$asset_path = apply_filters( 'wpld_cmb2_field_select2_asset_path', plugins_url( '', __FILE__  ) );

		wp_enqueue_script( 'wpld-taxonomy-location-script', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'wpld-select2', 'jquery-ui-sortable' ), self::VERSION );
		wp_localize_script( 'wpld-taxonomy-location-script', 'location_opts', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'ajaxurl_endpoint'      => WP_Listings_Directory_Ajax::get_endpoint(),
			'ajax_nonce' => wp_create_nonce( 'wpld-ajax-nonce' )
		));
		wp_enqueue_style( 'wpld-taxonomy-location-style', $asset_path . '/css/style.css', array( 'wpld-select2' ), self::VERSION );
	}
    ...
    public function process_change_location() {
    	check_ajax_referer( 'wpld-ajax-nonce', 'security' );
...
$wpld_cmb2_field_select2 = new WP_Listings_Directory_CMB2_Field_Taxonomy_Location();
