...
class PW_CMB2_Field_Taxonomy_Select2 {
	...
	public function render_pw_taxonomy_select( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		$condition = '';
		if ( !empty($attrs['data-condition']) ){
			$condition = $attrs['data-condition'];
		}
		echo $field_type_object->select( array(
			'class'            => 'pw_taxonomy_select2 pw_taxonomy_select '.($condition ? 'has-condition' : ''),
			'desc'             => $field_type_object->_desc( true ),
			'options'          => $this->get_pw_taxonomy_options( $field_escaped_value, $field_type_object, $condition ),
	...
	public function render_pw_taxonomy_multiselect( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		$a = $field_type_object->parse_args( 'pw_taxonomy_multiselect', array(
			'multiple'         => 'multiple',
			'style'            => 'width: 99%',
			'class'            => 'pw_taxonomy_select2 pw_taxonomy_multiselect '.($condition ? 'has-condition' : ''),
	 ...
	 */
	public function get_pw_taxonomy_options( $field_escaped_value = array(), $field_type_object, $condition ) {
		...
		foreach ( $options as $option_value => $option_label ) {
			...
			if ( $condition ) {
				...
				$type_slugs = get_term_meta( $option_value, '_category_parent', true );
				if ( $type_slugs ) {
					foreach ($type_slugs as $slug) {
						$term = get_term_by('slug', $slug, $condition);
						if ( $term ) {
							$attr .= ' condition-'.$term->term_id;
						}
					}
			...
			if ( in_array( $option_value, (array) $field_escaped_value ) ) {
				...
				$selected_items .= '<option value="'.$option_value.'" selected="selected" '.$attr_html.'>'.$option_label.'</option>';
			} else {
				// $other_items .= $field_type_object->select_option( $option );
				$selected_items .= '<option value="'.$option_value.'" '.$attr_html.'>'.$option_label.'</option>';
			}
		}
	...
	public function setup_admin_scripts() {
		...
		wp_enqueue_script( 'pw-taxonomy-select2-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'wpld-select2', 'jquery-ui-sortable' ), self::VERSION );
		wp_enqueue_style( 'pw-taxonomy-select2-tweaks', $asset_path . '/css/style.css', array( 'wpld-select2' ), self::VERSION );
	}
